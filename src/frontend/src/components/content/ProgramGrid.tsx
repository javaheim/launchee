...
import {ProgramButtonWithTooltip} from "./ProgramButtonWithTooltip.tsx";
import {HelpWithTooltip} from "@/components/content/HelpWithTooltip.tsx";
import {frontend} from "../../../wailsjs/go/models.ts";

export function ProgramGrid({content, programs}: Readonly<{
    content: frontend.Content | null,
    programs: frontend.Program[]
}>) {
    ...
    return (
        <div className={`grid ${iconColumnsClass} place-items-center-safe ${marginClass} ${gapClass}`}>
            {programs.length > 0 ? (programs.map((program) => (
                <ProgramButtonWithTooltip key={program.Id}
                                          program={program}
                                          iconSize={iconSize}/>
            ))) : (
                <HelpWithTooltip/>
